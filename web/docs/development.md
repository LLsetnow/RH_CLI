...
资源库初始化必须通过 `POST /api/resource-library` 或设置页“新建资源库”执行；目标目录必须为空，初始化器负责生成 `Resources.json` 和所有空索引。
...
- 不把用户原始输入文件复制进项目目录；需要落盘的输入统一保存到 `data/input/<来源>/`，剪贴板图片位于 `data/input/pasted/`。
