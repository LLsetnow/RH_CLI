...
- `prompt_store.py`、`action_store.py`、`reference_store.py` 分别管理提示词、动作和参考资源的本地索引；动作/参考资源统一从设置的媒体库 ref 根目录读取固定的五个子目录和 Markdown 文件。每个条目保存稳定 `id:`、`tags:`、文本和相对媒体路径，前端编辑后由对应 Store 原子重写 Markdown，缓存只作为派生数据。动作 Markdown 的 `###` 标题作为动作一级分类，条目 `tags:` 作为二级标签，解析时会去掉重复的一级分类标签。
...
- `data/outputs/`：默认产物目录；用户可以在设置中指定外部绝对路径。每个任务文件夹保存 `workflow_api.json` 和可选的 `prompt_group.json` 提示词工坊组状态快照。
...
每个任务的产物目录会保存当前 API 工作流快照，通常为 `<输出目录>/<task_id>/workflow_api.json`，以及提交时当前组装台的 `prompt_group.json`。任务加载接口会同时返回两个快照；前端通过一次性本地缓存把提示词组状态恢复到提示词工坊，旧任务没有该文件时保持兼容。
