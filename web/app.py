...
def inspect_workflow(workflow: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"file_inputs": files, "prompt_inputs": prompts, "file_count": len(files), "prompt_count": len(prompts)}
...
class LocalStore:
    ...
    def create_task(self, task: dict[str, Any]) -> None:
        ...
        with self._lock:
            self._db.execute(
                "INSERT INTO tasks (id,created_at,updated_at,status,progress,workflow_path,workflow_name,key_id,remote_task_id,"
                "input_json,prompt_json,output_dir,outputs_json,error,cost_type,cost,duration) "
                "VALUES (:id,:created_at,:updated_at,:status,:progress,:workflow_path,:workflow_name,:key_id,:remote_task_id,"
                ":input_json,:prompt_json,:output_dir,:outputs_json,:error,:cost_type,:cost,:duration)",
                fields,
            )
            self._db.commit()
    ...
    def update_task(self, task_id: str, **changes: Any) -> None:
        allowed = {
            "status", "progress", "updated_at", "started_at", "completed_at", "key_id", "remote_task_id",
            "outputs_json", "error", "cost_type", "cost", "duration", "output_dir",
        }
    ...
    def row_to_task(row: sqlite3.Row) -> dict[str, Any]:
        task = dict(row)
        names = {"input_json": "files", "prompt_json": "prompts", "outputs_json": "outputs"}
...
class TaskManager:
    ...
    def check_key(self, key_id: str) -> dict[str, Any]:
        ...
        try:
            with RhHttpClient(record["api_key"], timeout=15.0, no_proxy_host="runninghub.ai" if site == "ai" else "") as client:
    ...
    def _run_task(self, task_id: str, key: dict[str, Any], cancel_event: threading.Event) -> None:
        try:
            ...
            self.store.update_task(task_id, status=status, progress=exc.message, error=exc.message, completed_at=now_ms())
        except Exception as exc:  # pragma: no cover - final safety net for background jobs
            self.store.update_task(task_id, status="failed", progress="任务失败", error=str(exc), completed_at=now_ms())
        finally:
            with self._lock:
