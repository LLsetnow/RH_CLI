...
- `data/outputs/`：默认输出目录下的任务产物；每个 `<task_id>/` 还会保存 `workflow_api.json` 和提交时提示词工坊组装台的 `prompt_group.json`
...
- 每次提交都会在任务产物目录的 `<task_id>/workflow_api.json` 保存当前 API 工作流快照，并保存 `prompt_group.json` 作为提示词工坊组状态快照；加载任务时优先读取这些快照，旧任务在原始工作流仍存在时会自动补写工作流快照。成片库中点击工作流名称也会同步准备提示词组状态，进入提示词工坊即可恢复。
...
- 顶部“工作流”页面只显示用户在该页面明确导入并保存的 API JSON，并按账号分组；任务提交页和历史任务产生的临时工作流副本不会自动进入工作流库。页面支持搜索、导入、绑定账号和 workflowId，并可在“编辑工作流资料”中查看、编辑和保存 JSON，也支持导出和删除工作流库副本；选择工作流后可手动从导航进入任务提交页。删除工作流不会删除任务历史和产物。
...
- 提示词页面的“积木库”可以切换“基础积木”、动作、人物、音频、背景和服装。媒体库统一以 `VideoMake/ref` 为根目录，读取 `pose/pose.md`、`character/character.md`、`audio/audio.md`、`background/background.md` 和 `clothes/clothes.md`；设置页只需配置一次 ref 文件夹。动作/参考卡片支持媒体预览、文本预览和标签编辑，保存后会回写对应 Markdown。
...
- 基础积木默认读取 `VideoMake/ref/prompt/library.md`。文件使用 `##` 分组、`####` 定义积木，随后写 `id:`、`tags:` 和 `>` 提示词正文；内容按 MiniMax H3 的模式对齐、参考定义、镜头动作、摄影机、声音、对白和音乐字段拆分，拖入工作台后按顺序拼接。设置中可以选择其他 Markdown 文件；修改后刷新提示词工坊即可重新抽取。点击基础积木左上角名称可编辑，新增或编辑后都会回写该 Markdown 文件，删除按钮也会同步删除对应条目。
